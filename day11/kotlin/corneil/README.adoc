...
fun parseMonkey(lines: List<String>): Monkey {
  ...
  val lambda: (Long) -> Long = if (isAdd) { old -> Math.addExact(old, constant ?: old) } else { old -> Math.multiplyExact(old, constant ?: old) } // <1>
...
fun processItems(monkeys: Map<Int, Monkey>, rounds: Int, divisor: Long = 3L): Map<Int, Monkey> {
  val divisors = monkeys.values.map { monkey -> monkey.worriedLevel }.reduce { acc, l -> acc * l * divisor } // <1>
