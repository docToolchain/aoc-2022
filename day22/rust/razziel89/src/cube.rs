...
fn affected_fields(
    ...
    // let ref_neigh_data = neigh_data
    //     .iter()
    //     .find(|el| el.pos == ref_point)
    //     .ok_or(Error::msg("cannot find ref point"))?;
    // let blocking_field = ref_neigh_data.field;
    ...
    let maybe_start_neigh_data = neigh_data.iter().find(|el| {
        // println!("{:?} <> {:?}", *el.pos, start_neigh_point);
        *el.pos == start_neigh_point
    });
...
pub fn build<'b>(
    ...
    // let ref_points_idx = ref_points
    //     .iter()
    //     .flat_map(|el| points.iter().position(|p| p == el))
    //     .collect::<Vec<_>>();
    // assert_eq!(ref_points_idx.len(), ref_points.len());

    // for rp_idx in ref_points_idx.iter() {
    //     println!("{}", points_3d[*rp_idx].pos);
    // }
    ...
    for idx in 0..field_neigh.len() {
        ...
        for is_right in [true, false] {
            ...
            // println!("affected {:?}", affected_field_idx);
...

            {
                ...
                let rot_ax = as_unit(&field_point_3d.norm.cross(&diff)).context("axis")?;
                let ang = 90;
                trans = as_unit(&-diff).with_context(|| {
                    format!(
                        "translation from {} to {}",
            ...
            points_3d = points_3d
                ...
                .map(|(idx, el)| {
                    ...
                    // println!("affected? {} <> {:?}", field_idx, affected_field_idx);
