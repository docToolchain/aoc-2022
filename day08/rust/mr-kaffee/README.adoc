...
Ans this is the first puzzle this year, that solves in > 1ms for me.

=== Input ===

I directly use the bytes from the input. I scan for the first occurance of a line break. Its index is the width of the grid.

The `PuzzeData` struct's implementation has an additional method to return the `height` of the grid.
